...
///
/// Save failures route through `RivenRootController.showBanner` so the
...
struct EditorPaneView: View {
    ...
    /// the app-wide RivenBanner the controller manages, so the
...
enum EditorChrome {
    /// 13 pt monospaced. Riven favors SF Mono when available, otherwise
