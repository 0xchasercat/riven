...
struct OverlayInputHeader<Trailing: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: RivenSpacing.s) {
                Text(leading)
                    .font(RivenType.mono(13, weight: .semibold))
                    .foregroundStyle(Color(hex: theme.chrome.accent.hex))
                TextField(placeholder, text: $text)
                    .textFieldStyle(.plain)
                    .font(RivenType.mono(15))
                    .foregroundStyle(Color(hex: theme.chrome.text.hex))
                Spacer(minLength: RivenSpacing.s)
                trailing
                    .font(RivenType.mono(11))
...
struct OverlayRow<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            .animation(RivenMotion.hover, value: isHighlighted)
...
struct OverlayPrimaryButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(RivenType.chrome(12, weight: .semibold))
...
struct OverlaySecondaryButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(RivenType.chrome(12, weight: .medium))
                .foregroundStyle(Color(hex: theme.chrome.text.hex))
                .padding(.horizontal, RivenSpacing.m)
                .padding(.vertical, RivenSpacing.s)
                .background(
...
struct CommandPaletteOverlay: View {
    ...
    private var emptyState: some View {
        VStack(spacing: RivenSpacing.xs) {
                    ...
                    ForEach(Array(rows.enumerated()), id: \.offset) { _, row in
                        ...
                        case .header(let group):
                            SectionLabel(theme: theme, group)
                                .padding(.horizontal, RivenSpacing.l)
                                .padding(.top, RivenSpacing.m)
                                .padding(.bottom, RivenSpacing.xs)
                                .frame(maxWidth: .infinity, alignment: .leading)
...
struct SearchOverlay: View {
    ...
    /// by the parent (RivenRootController) which creates a new
    ...
    private func scopePill(label: String, value: SearchScope) -> some View {
        ...
        Button {
            ...
            Text(label)
                .font(RivenType.mono(11, weight: isActive ? .semibold : .regular))
    ...
    private var recentSearchesBody: some View {
        if recent.isEmpty {
            ...
            .padding(.bottom, RivenSpacing.s)
        }
    }

    private func emptyHint(_ text: String) -> some View {
        Text(text)
            .font(RivenType.chrome(12))
            .foregroundStyle(Color(hex: theme.chrome.tertiaryText.hex))
            .frame(maxWidth: .infinity)
            .padding(.vertical, RivenSpacing.xxl)
    }
    ...
    var body: some View {
        HStack(spacing: RivenSpacing.s) {
            ...
            Text(filenameComponent)
                .font(RivenType.mono(12, weight: .semibold))
                .foregroundStyle(Color(hex: theme.chrome.text.hex))
                .lineLimit(1)
            Spacer(minLength: RivenSpacing.s)
...
private struct FileHitRow: View {
    ...
    private func lineNumberGutter(_ number: Int, isHit: Bool) -> some View {
        Text("\(number)")
            .font(RivenType.mono(10, weight: isHit ? .semibold : .regular))
...
private struct ScrollbackHitRow: View {
    ...
    var body: some View {
        OverlayRow(theme: theme, isHighlighted: isHighlighted, height: 44) {
            HStack(alignment: .firstTextBaseline, spacing: RivenSpacing.m) {
            ...
            HStack(spacing: RivenSpacing.s) {
                ...
                if isHovered {
                    Button(action: onRemove) {
                        Text("×")
                            .font(RivenType.mono(12, weight: .medium))
                            .foregroundStyle(Color(hex: theme.chrome.tertiaryText.hex))
    ...
    var body: some View {
        ...
        return ZStack(alignment: .top) {
            ...
            content
                ...
                .shadow(
                    color: RivenElevation.modal.color,
                    radius: RivenElevation.modal.radius,
                    x: RivenElevation.modal.x,
                    y: RivenElevation.modal.y
                )
