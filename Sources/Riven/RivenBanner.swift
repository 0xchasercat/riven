...
struct RivenBannerState: Equatable, Identifiable {
    let id: UUID
    let message: String
    let kind: BannerKind
...
/// `RivenRootView.mainColumn`. Themed via `theme.chrome` so each
...
/// this component.
struct RivenBanner: View {
    let theme: ThemeSpec
    let state: RivenBannerState
    let onDismiss: () -> Void
...
/// Host view used by `RivenRootView` to wrap the banner in its
/// auto-dismiss `.task(id:)` so the chrome strip can be rebuilt
/// per-banner without the parent view managing a Task handle.
struct RivenBannerHost: View {
    let theme: ThemeSpec
    let state: RivenBannerState
    let onDismiss: () -> Void

    var body: some View {
        RivenBanner(theme: theme, state: state, onDismiss: onDismiss)
            .task(id: state.id) {
