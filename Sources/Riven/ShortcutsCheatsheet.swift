...
/// can hit from anywhere in Riven. Triggered by the Help menu's
/// "Riven Keyboard Shortcuts" item (⌘?). Sits on the same backdrop
    ...
    var body: some View {
        OverlayBackdrop(theme: theme, width: OverlayWidth.picker, onClose: onClose) {
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: RivenSpacing.l) {
                        ForEach(Array(rows.enumerated()), id: \.offset) { _, pair in
                            HStack(alignment: .top, spacing: RivenSpacing.xxl) {
                                column(pair.left)
                ...
                OverlayFooter(theme: theme) {
                    Text("Help \u{2192} About Riven for version info")
                }
            }
        }
