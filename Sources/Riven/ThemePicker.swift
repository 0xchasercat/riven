...
struct ThemePicker: View {
    ...
    private var header: some View {
        VStack(spacing: 0) {
            HStack(alignment: .top, spacing: RivenSpacing.m) {
    ...
    private var swatches: some View {
        ...
        return VStack(spacing: RivenSpacing.m) {
...
private struct ThemeSwatch: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: RivenSpacing.s) {
            preview
            HStack(alignment: .firstTextBaseline, spacing: RivenSpacing.xs) {
                Text(option.name)
                    .font(RivenType.chrome(13, weight: .semibold))
                    .foregroundStyle(Color(hex: theme.chrome.text.hex))
                if isCustom {
                    Text("custom")
                        .font(RivenType.mono(9, weight: .semibold))
                        .tracking(0.4)
                ...
                Spacer(minLength: RivenSpacing.xs)
                Text("\(index)")
                    .font(RivenType.mono(10, weight: .semibold))
                    .foregroundStyle(Color(hex: theme.chrome.tertiaryText.hex))
        ...
        .padding(RivenSpacing.s)
        .frame(width: Self.tileWidth, alignment: .leading)
        .background(
            RoundedRectangle(cornerRadius: RivenRadius.medium, style: .continuous)
        ...
        .animation(RivenMotion.hover, value: isSelected)
    }
    ...
    private var preview: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: RivenRadius.small, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: RivenRadius.small, style: .continuous)
