...
final class ScrollbackPersister: @unchecked Sendable {
    ...
    init(
        ...
        self.ioQueue = DispatchQueue(label: "riven.agent.scrollback-persister.io", qos: .utility)
    ...
    func flush(paneID: PaneID) {
        ...
        do {
            ...
            FileHandle.standardError.write(Data("RivenAgent scrollback flush failed for \(paneID): \(error)\n".utf8))
        }
    }
...
actor AgentBroker {
    ...
    /// the same Riven launch" without needing wall-clock heuristics.
    ...
    func createPane(
        ...
        if let persister {
            ...
            do {
                ...
                FileHandle.standardError.write(Data("RivenAgent metadata seed failed for \(paneID): \(error)\n".utf8))
            }
        }
...
struct RivenAgentMain {
    ...
    static func main() async {
        ...
        do {
            ...
            FileHandle.standardError.write(Data("RivenAgent fatal: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    static func runAgent(socketPath: String, scrollbackRoot: URL?) async throws {
        ...
        listener.stateUpdateHandler = { state in
            ...
            case let .failed(error):
                FileHandle.standardError.write(Data("RivenAgent listener failed: \(error)\n".utf8))
        ...
        for await _ in started.stream {
            FileHandle.standardOutput.write(Data("RivenAgent listening on \(socketPath)\n".utf8))
            break
        }
    ...
    private static func defaultScrollbackRoot() -> URL {
        ...
        return base.appendingPathComponent("Riven", isDirectory: true).appendingPathComponent("scrollback", isDirectory: true)
    }
    ...
    private static func installShutdownHandlers(persister: ScrollbackPersister) {
        let queue = DispatchQueue(label: "riven.agent.shutdown", qos: .userInitiated)
