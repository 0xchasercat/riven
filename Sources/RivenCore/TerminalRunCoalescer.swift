...
/// rather than as `NSColor` so this type can live in `RivenCore`, which
...
/// Pure, AppKit-free run coalescer. Lives in `RivenCore` so the renderer
/// in `Sources/Riven/` can call it AND so the test target can exercise
