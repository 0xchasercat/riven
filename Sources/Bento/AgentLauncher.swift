...
/// Spawns and supervises the `BentoAgent` broker subprocess so the UI can
...
///      one we leave it running so other Bento windows keep working.
///
...
public final class AgentLauncher {
    ...
    public enum LaunchError: Error, CustomStringConvertible {
        ...
        public var description: String {
            switch self {
            case .binaryNotFound: return "BentoAgent binary not found near app bundle or via swift build --show-bin-path"
            case .agentDidNotStart: return "BentoAgent spawned but did not become connectable within timeout"
            }
        }
    }
    ...
    /// Locate the BentoAgent executable. Order:
    ...
    static func locateAgentBinary() throws -> URL {
        ...
        if let exe = Bundle.main.executableURL {
            let candidate = exe.deletingLastPathComponent().appendingPathComponent("BentoAgent")
