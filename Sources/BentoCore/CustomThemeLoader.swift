...
/// `~/Library/Application Support/Bento/themes/*.json`.
///
...
public final class CustomThemeLoader: @unchecked Sendable {
    ...
    private static func scan(directory: URL) -> [ThemeSpec] {
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data("[bento] custom themes: scan failed: \(error)\n".utf8)
            )
            return []
        }
        ...
        for url in entries where url.pathExtension.lowercased() == "json" {
            do {
                ...
                FileHandle.standardError.write(
                    Data("[bento] custom theme \(url.lastPathComponent) failed to load: \(error)\n".utf8)
                )
            }
        }
    ...
    public static func seedDefaultTemplate(directory: URL = CustomThemeLoader.defaultDirectory()) -> URL? {
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data("[bento] custom themes: mkdir failed: \(error)\n".utf8)
            )
            return nil
        }
        let target = directory.appendingPathComponent("bento-default.json")
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data("[bento] custom themes: template write failed: \(error)\n".utf8)
            )
            return nil
        }
    }
