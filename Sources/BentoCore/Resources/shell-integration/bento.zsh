# Bento shell integration — main entry point.
#
# This file is the entry point a user's ~/.zshrc sources. It loads the
# rest of the Bento integration in a deterministic order:
#
#   1. options.zsh   sensible zsh defaults (history, completion, etc.)
#   2. integration.zsh   OSC 7 / 133 hooks so Bento's sidebar + chrome
...
#                    own docs)
#
# Everything below is gated on `$BENTO` (or `$TERM_PROGRAM == Bento`)
# so this file is a NO-OP outside Bento. Sourcing it from another
...
# Identify ourselves. `$BENTO_INTEGRATION_VERSION` lets Bento's installer
...
export BENTO_INTEGRATION_VERSION="1"

# Run-only-inside-Bento gate. Bento sets `TERM_PROGRAM=Bento` on every
# PTY spawn (Sources/Bento/TerminalPaneView.swift). Other terminals
# don't, so sourcing this from a non-Bento shell is a clean no-op.
if [[ "${TERM_PROGRAM:-}" != "Bento" ]]; then
  return 0
fi
...
typeset -g BENTO_INTEGRATION_DIR="${BENTO_INTEGRATION_DIR:-${${(%):-%N}:A:h}}"

# Load order matters. Each loader is small + idempotent.
for _bento_part in options integration prompt keybinds plugins; do
  if [[ -r "$BENTO_INTEGRATION_DIR/$_bento_part.zsh" ]]; then
    source "$BENTO_INTEGRATION_DIR/$_bento_part.zsh"
  fi
done
unset _bento_part

# Mark the integration as live so other tooling (e.g. a `bento doctor`
# script) can probe.
typeset -g BENTO_INTEGRATION_LOADED=1
