# Bento shell integration — minimal, theme-aware prompt.
#
# Two lines of visible chrome, designed to match Bento's compartment
# aesthetic:
#
#     ~/code/ bento · main ✗
#     ›
...
#   * Prompt character is the theme's prompt color (Bento amber,
#     Tokyo violet, etc.). Goes red after a non-zero exit so the
#     user notices a failure without scrolling up.
#
# Colors are 256-color ANSI escapes that flow through Bento's tuned
# palette (Sources/BentoCore/ThemeSpec.swift). That means the prompt
# automatically follows the active theme — switching from Bento to
# Tokyo in Bento's status-bar swatch re-tints the prompt without the
# shell reloading.
...
# Map to ANSI palette indices that Bento's themes interpret:
...
#   warn        →  index 3  (yellow)
#   err         →  index 1  (red)
typeset -g _BENTO_C_DIM='%F{8}'
typeset -g _BENTO_C_ACCENT='%F{3}'
typeset -g _BENTO_C_OK='%F{2}'
typeset -g _BENTO_C_WARN='%F{3}'
typeset -g _BENTO_C_ERR='%F{1}'
typeset -g _BENTO_C_RESET='%f'
...
typeset -g _BENTO_GIT_STATUS_DIR="${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}/bento-git-$$"
mkdir -p "$_BENTO_GIT_STATUS_DIR"
trap "rm -rf '$_BENTO_GIT_STATUS_DIR'" EXIT

# Last-seen pwd + last-seen branch so we can invalidate the cache
# when the user `cd`s into a different repo.
typeset -g _BENTO_LAST_GIT_PWD=""
typeset -g _BENTO_GIT_STATUS=""

_bento_git_probe_async() {
  local pwd_snapshot="$PWD"
  local outfile="$_BENTO_GIT_STATUS_DIR/status"
  (
...
_bento_prompt_precmd() {
  ...
  if [[ "$PWD" != "$_BENTO_LAST_GIT_PWD" ]]; then
    _BENTO_LAST_GIT_PWD="$PWD"
    _BENTO_GIT_STATUS=""   # clear stale state immediately on cd
    rm -f "$_BENTO_GIT_STATUS_DIR/status"
    _bento_git_probe_async
  else
...
# Builds the right-hand "git" segment from `_BENTO_GIT_STATUS` (set
...
# break.
_bento_segment_git() {
  [[ -z $_BENTO_GIT_STATUS ]] && return
  local branch="${_BENTO_GIT_STATUS%%|*}"
  local state="${_BENTO_GIT_STATUS#*|}"
...
# Right-side prompt: kept empty for now — Bento's status bar already
