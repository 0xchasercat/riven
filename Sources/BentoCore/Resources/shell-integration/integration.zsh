# Bento shell integration — terminal escape sequence hooks.
#
# Two flavors of OSC (Operating System Command) sequences feed Bento's
# chrome state:
#
#   OSC 7  — "the shell is now in directory X"
#            Bento's BrokeredTerminalView reads this in its draw loop
...
#            These let Bento (and any future "jump to previous
...
#     where no command has run yet — `_BENTO_PRECMD_RAN` gates this)
#   * the cwd report (OSC 7)
#   * A to mark the new prompt's start
_bento_precmd() {
  _BENTO_LAST_EXIT=$?
  if (( ${+_BENTO_PRECMD_RAN} )); then
    _bento_osc133_cmd_end "$_BENTO_LAST_EXIT"
  fi
  typeset -g _BENTO_PRECMD_RAN=1
  _bento_osc7
...
# Print absolute paths with the OSC 8 hyperlink escape so Bento can
# turn them into ⌘-clickable links. Tools like `ls`, `git`, etc.
# print bare paths; users can opt into hyperlinking by piping through
# `bento-hyperlink`. We don't auto-wrap because rewriting every
