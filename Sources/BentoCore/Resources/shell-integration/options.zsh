# Bento shell integration — sensible zsh defaults.
#
...
# busy shell, and ripgrep through it (via Bento's global search,
...
# session — important because Bento's broker outlives the UI.
...
setopt NO_BEEP                # silent terminal — Bento has the banner
setopt PROMPT_SUBST           # required for the dynamic prompt below

# ─── Color env ────────────────────────────────────────────────────
# Bento sets TERM=xterm-256color + COLORTERM=truecolor on every PTY
# spawn. Make sure common tools pick that up.
export CLICOLOR=1
export LESS="${LESS:-FRX}"    # raw-control-chars, no-init, quit-if-one-screen
export PAGER="${PAGER:-less}"

# Bento's compartment chrome is dark by default. Tell ls to use a
