...
struct ShellIntegrationInstallerTests {
    ...
    private func makeInstaller(
        ...
        let dest = root.appendingPathComponent(".config/bento/shell", isDirectory: true)
        ...
        // Always include bento.zsh — that's the entry-point file
        // isInstalled() checks for after the marker.
        var files = bundleContents
        files["bento.zsh"] = files["bento.zsh"] ?? "# Bento entry\n"
    ...
    func freshInstall() throws {
        ...
        #expect(zshrcBody.contains("bento.zsh"))

        // bento.zsh landed in the destination.
        let entry = installer.destinationDirectory.appendingPathComponent("bento.zsh")
    ...
    func injectReplacesPrevious() throws {
        ...
        source /old/path/bento.zsh
        \(ShellIntegrationInstaller.endMarker)
        # tail
        """
        let fresh = """
        \(ShellIntegrationInstaller.beginMarker)
        source /new/path/bento.zsh
        \(ShellIntegrationInstaller.endMarker)
        """
