...
struct AgentIPCTests {
    /// End-to-end: spawn the RivenAgent binary as a subprocess, connect via
...
/// Builds and launches the RivenAgent product as a subprocess on a unique
...
final class AgentHarness {
    ...
    static func start() async throws -> AgentHarness {
        ...
        let socketURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("riven-agent-\(UUID().uuidString).sock")
    ...
    private static func locateAgentBinary() throws -> URL {
        // The test bundle binary lives next to the RivenAgent product binary
        ...
        for dir in candidates {
            let candidate = dir.appendingPathComponent("RivenAgent")
