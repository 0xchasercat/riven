...
struct WorkspaceControllerTests {
    ...
    func opensProjectWithUntrustedTasks() async throws {
        ...
        try FileManager.default.createDirectory(at: project.appendingPathComponent(".riven"), withIntermediateDirectories: true)
        try """
        version: 1
        panes:
          - name: api
            cwd: backend
            cmd: cargo run
        """.write(to: project.appendingPathComponent(".riven/session.yml"), atomically: true, encoding: .utf8)
    ...
    func openProjectFallsBackOnMissingRoot() async throws {
        ...
        let missing = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("riven-missing-\(UUID().uuidString)")
